$(function () {
  ...
  input.addEventListener("change", function (evt) {
    ...
    if (formdata) {
      $.ajax({
        url: "http://192.168.1.10:8888/GNC/lib/php/upload.php",
        type: "POST",
        data: formdata,
        processData: false,
        contentType: false,
          ...
          $("#crop").on("click", function(){
              ...
              function drawImage(imageObj) {
                ...
                for(var i = 0; i < data.length; i += 4) {
                  var brightness = 0.3 * data[i] + 0.59 * data[i + 1] + 0.51 * data[i + 2];
                  // red
                  data[i] = brightness;
                  // green
                  data[i + 1] = brightness;
  ...
  $('#btnEnterTxt').click(function(){
    // draw rect frame and embed text image
    var txtCanvas = document.getElementById('txtCanvas');
    var ctx = txtCanvas.getContext('2d');   

    ctx.fillStyle = "rgba(42,168,227, 0.40)"
    ctx.fillRect(167, 20, 203, 166);

    var txtOverlay = new Image();

    txtOverlay.onload = function() {
      ctx.drawImage(txtOverlay, 175, 30);
    };
    txtOverlay.src = 'http://192.168.1.10:8888/GNC/assets/images/txt_overlay.png';
  ...
  $('#prevStepOne').click(function(){
    $('#step2').animate({left: "-9999"}, 250, function(){ // causes upload button to fail somehow
 ...
 $('#btnAddText').click(function(){
    // SAVE FINAL IMAGE FUNCTIONALITY
    var imgBase64 = "";
    imgBase64 = document.getElementById("txtCanvas").toDataURL("image/png");
    imgBase64.replace(/^data:image\/(png|jpg);base64,/, ""); // add correct uri

    $.ajax({
      type: "POST",
      url: "lib/php/save_image.php",
      data: { 
        dataURL: imgBase64
      }
    ...
    // display returned url of generated insta-graph to page
    function returnImage(response) {
        $('#finalImage').attr('src', 'http://192.168.1.10:8888/GNC/assets/final/' + response);
        $('#previewShare').attr('src', 'http://192.168.1.10:8888/GNC/assets/final/' + response);    
    }
